...
-(void)loadObjects
{
	...
	for (NSString *line in [manager contentsOfDirectoryAtPath:@"/User/Documents/DesignerXI" error:nil])
	{
		...
		{
			Project *new = [Project alloc];
			new.type = [[line substringToIndex:1] intValue];
			new.name = [line substringFromIndex:1];
...
-(void)loadThemeObjects
{
	...
	{
		line = [line substringToIndex:[line length] - 7];
		[_themeObjects insertObject:line atIndex:0];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	{
		cell.textLabel.text = _themeObjects[indexPath.row];
	}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (tableView == self.tableView)
	{
		...
		if ([self getCurrentProject].type == 0)
		{
			...
			self.navigationItem.leftBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(backToHome)] autorelease];
		...
		else
		{
			[self.tableView setScrollEnabled:NO];
			self.navigationItem.leftBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(backToHome)] autorelease];
			self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(addThemeBundle)] autorelease];
	...
	else
	{
		...
		self.navigationItem.leftBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(backToThemeEditView)] autorelease];
		self.navigationItem.rightBarButtonItem = [[[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemTrash target:self action:@selector(trashButtonREEE)] autorelease];
...
-(void)addThemeBundle
{
	UIAlertController *nameAlert = [UIAlertController alertControllerWithTitle: @"DesignerXI" message:@"Enter the bundle ID of the app:" preferredStyle:UIAlertControllerStyleAlert];
	[nameAlert addTextFieldWithConfigurationHandler:^(UITextField *textField)
    {
...
-(void)buildCurrentProject:(BOOL)install
{
	...
	if ([manager fileExistsAtPath:[[self getCurrentProjectDirectory:YES] stringByAppendingString:@"/control"]])
	{
		...
		if (install)
		{
			...
			if (![retStr isEqual:@""] && ![retStr containsString:@"anemone"])
			{
				[self error:@"Error" message:retStr];
				return;
			}
		}
		[self error:@"Success" message:(install ? @"Project successfully built at /var/mobile/Documents/DesignerXI/package.deb.":@"Project successfully installed.")];
		return;
	}
	else
	{
